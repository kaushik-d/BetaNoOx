...
CreateErrorHandler(PETScUnsymmSparseMatrix);
//=============================================================
PETScUnsymmSparseMatrix::PETScUnsymmSparseMatrix(const int numEqns) :
		LargeMatrix(numEqns)
{
	WhoClass("PETScUnsymmSparseMatrix");
	WhoMethod("PETScUnsymmSparseMatrix(const int)");
...
//=============================================================
PETScUnsymmSparseMatrix::~PETScUnsymmSparseMatrix()
{
	DeleteWhoMethod("~PETScUnsymmSparseMatrix()");
//	cerr << "Deleting PETScUnsymmSparseMatrix matrix" << endl;
...
//=============================================================
void PETScUnsymmSparseMatrix::binaryWrite(FILE *outfile)
{
...
//=============================================================
void PETScUnsymmSparseMatrix::binaryRead(FILE *infile)
{
...
//=============================================================
double& PETScUnsymmSparseMatrix::operator()(const int ii,
		const int jj)
{
...
//=============================================================
void PETScUnsymmSparseMatrix::setaij(const int i, const int j, double val)
{
	...
	if(i==j){
		if(FilledDiagonal[i]==false){
			...
			if(val != 1.0){
				cerr << "you are trying to reset a diagonal value from 1.0 to another value. currently,(";
				cerr << i << "," << i << ")=1.0 you are trying to re-write it to " << val << ".This is not allowed with the ";
				cerr << "logic currently in place. Check your global stiffness matrix assembly!" << endl;
				exit(1);
			}
		}
	}else{
		cout << "PETScUnsymmSparseMatrix::setaij() can be used only to set diagonal terms!" << endl;
		exit(1);
	}
}
//=============================================================
void PETScUnsymmSparseMatrix::addtoaij(const int i, const int j, double val)
{
	if(i>=numberOfEquations || j>=numberOfEquations ) {
		cerr << "numberOfEquations = " << numberOfEquations << endl;
		WhoMethod("setaij(const int i, const int j, double val)");
...
//=============================================================
double PETScUnsymmSparseMatrix::getaij(const int i, const int j)
{
	PetscScalar val;
...
//=============================================================
void PETScUnsymmSparseMatrix::solve(double *b,double *initialGuess) 
{
...
cout <<"Iterations for convergence="<< num_of_iter << " - Residual Norm = " << r_norm << endl;
	...
	ierr = KSPView(ksp,PETSC_VIEWER_STDBETA_OUT_WORLD);//CHKERRQ(ierr); //instead of -ksp_view command line option 
    KSPDestroy(ksp);
    VecDestroy(rhs);
    VecDestroy(x);
...
//=============================================================
void PETScUnsymmSparseMatrix::zeroYourself(void)
{
	MatZeroEntries(mat);
}
//=============================================================
void PETScUnsymmSparseMatrix::printMatrix(char *message)
{
	cout << message << "\nPETScUnsymmSparseMatrix with " << numberOfEquations <<
		" equations.\n";
	/*
	for(i=0;i<L;i++) {
...
//=============================================================
void PETScUnsymmSparseMatrix::printMatrixAddressFormat(char * message, ostream* out)
{ 
	//*out << message <<endl; 
...
#undef FORMAT 
//=============================================================
void PETScUnsymmSparseMatrix::readMatrixAddressFormat(istream* instream)
{ 
	int numNonZeros;
...
//=============================================================
void PETScUnsymmSparseMatrix::vectorProduct(double *result, double *vector)
{
	/*
	int i,j,nt;
	double *mRow;
...
//=============================================================
void PETScUnsymmSparseMatrix::specifyNonZeroLocation(const int I, const int J)
{ 
	//this is the aij format (general matrix)
	row[I].add(J); 
	row[J].add(I); 
}
//=============================================================
void PETScUnsymmSparseMatrix::allocate(void)
{
	const int L=numberOfEquations;
    ...
    //MatCreateSeqAIJ(PETSC_COMM_SELF, L, L, PETSC_DEFAULT, nnz, &mat);
	
	MatCreate(PETSC_COMM_WORLD,&mat);
	MatSetSizes(mat,PETSC_DECIDE,PETSC_DECIDE,L,L);
	MatSetType(mat,MATAIJ);
    MatSeqAIJSetPreallocation(mat, nnz);
...
//=============================================================
#ifdef DEBUG_PETScUnsymmSparseMatrix
ofstream Logfile;
void main(void)
{
	int i;
	...
	int NumEquations;
	is >> NumEquations;
	PETScUnsymmSparseMatrix A(NumEquations);
